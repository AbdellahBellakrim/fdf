...
/*   Updated: 2022/02/27 12:45:34 by abellakr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_function_bonus(t_fdf_var *vars)
{
	...
	my_zoom_bonus(vars);
	vars->ptr->window_ptr = mlx_new_window \
	(vars->ptr->mlx_ptr, WEIGHT, HEIGH, "fdf");
	vars->ptr->image_ptr = mlx_new_image \
	(vars->ptr->mlx_ptr, WEIGHT, HEIGH);
	vars->ptr->buffer = mlx_get_data_addr (vars->ptr->image_ptr, \
	&vars->ptr->bpp, &vars->ptr->line_lenght, &vars->ptr->endian);
...
int	keyboard_bonus(int key, t_fdf_var *number)
{
	...
	mlx_put_image_to_window (number->ptr->mlx_ptr, \
	number->ptr->window_ptr, number->ptr->image_ptr, 0, 0);
